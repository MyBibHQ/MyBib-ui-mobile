...
export default class Project {
  ...
  async deleteReference (reference) {
    ...
    const removedReference = this.references.splice(referenceIndex, 1)[0];

    try {
      await axios.delete(BASE_API_URL + '/projects/' + this.id + '/references/' + reference.id, { headers: { 'Access-Token': AccessToken.get() } })
      this._lastDeletedReference = {
        index: referenceIndex,
        reference: removedReference
      };
    } catch (error) {
      // it failed. Put it back.
      this.references.splice(referenceIndex, 0, removedReference);
    }
  }

  restoreLastDeletedReference () {
    const lastDeletedReference = this._lastDeletedReference;
    this.references.splice(lastDeletedReference.index, 0, lastDeletedReference.reference);

    axios.put(BASE_API_URL + '/projects/' + this.id + '/references/' + lastDeletedReference.reference.id + '/restore', { }, { headers: { 'Access-Token': AccessToken.get() } })
      .then(() => {
        // it worked. Nothing to do
      }, () => {
        // it failed. remove it again
        this.references.splice(lastDeletedReference.index, 1);
      })

    this._lastDeletedReference = {};
    return lastDeletedReference.reference;
  }
}
